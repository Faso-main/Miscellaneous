...
def save():
    with open('table.csv', 'w+') as csv_file:
        ...
        csv_writer = csv.DictWriter(csv_file, fieldnames=fieldnames, delimiter='\t', newline='')
...
def load():
    try:
        with open('table.csv', 'r') as csv_file:
            fieldnames = ['name', "dept", 'role']
            csv_reader = csv.DictReader(csv_file, fieldnames=fieldnames, delimiter='\t', newline=None)
...
while True:
    ...
    if choice == 5: # таблица кафедр
        if check_table():
            ...
            print("Кафедра - кол-во преподавателей")
            for i in depts:
                amount = 0
                for j in table_list:
                    if i == j.dept:
